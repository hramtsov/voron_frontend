            ...
            <Table row-key="id" :columns="managers_columns" :data="managers">
                ...
                <template slot-scope="{ row, index }" slot="edit">
                    <Tooltip content="Редактировать" placement="top" :transfer="true">
                        <nuxt-link :to="`/managers/edit/${row.id}`" ><Icon custom="fad fa-pencil-alt gray-color mx-1 my-1" /></nuxt-link>
                    </Tooltip>
                </template>

                <template slot-scope="{ row, index }" slot="change-pass">
                    <Tooltip content="Изменить пароль" placement="top" :transfer="true">
                        <nuxt-link :to="`/managers/edit/${row.id}`" ><Icon custom="fad fa-key gray-color mx-1 my-1" /></nuxt-link>
                    </Tooltip>
                </template>
...
<script>
    export default {
        ...
        methods: {
            ...
            async load() {
                try {
                    let response = await this.$axios.$get('/managers/getlist', {progress: false})
