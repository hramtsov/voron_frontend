...
<!--                        <Button :type="( index == $route.params.table ? 'primary' : 'default' )" :to="`/cars/${index}`">-->
<!--                            {{ category.title }}-->
<!--                        </Button>-->
<!--                    </Badge>-->
<!--                </template>-->
...
<!--            </template>-->
      ...
      <button id="show-modal" @click="test = true">Show Modal</button>

      {{test}}
      <modal v-if="test" @close="test = false">
        ...
        <div class="card-box">
              ...
              <div class="tabs-cars">
                  <template v-for="(category, index) in tables" v-if="category.count_cars > 0">
                      <nuxt-link :to="`/cars/${index}`" :class="[( index == $route.params.table ? 'active' : '' )]">
                                    ...
                                    <td>
                                        <template v-if="type.first_car_id == car.id">
                                            <Tooltip content="Информация об авто" placement="top" :transfer="true">
                                                    <img @click="info_car(car.id)" class="car_img_mini"
                    ...
                    <span class="car_model_title">
                      ...
                      {{ type.subtitle }}
                                    ...
                                    <td>
                                        <template v-if="type.first_car_id == car.id">
                                            <Poptip trigger="hover">
                                                <span :id="`popover-count-subs-${car.id}`" v-if="type.subs_count > 0"
                                                      :class="['count_subs', (car.notifyDisable == 1 ? 'notify-opacity' : '')]">{{ type.subs_count }}</span>
                                    ...
                                    <td>
                                        <div v-if="car.parking.length != 0">
                                            ...
                                            <template v-if="car.parking.state == 'paid'">
                                                <Tooltip content="Оплачено" placement="top" :transfer="true">
                                                    <a v-if="$can('cars/parking')"
                                                       :data-remote='`/give/parkingpayed/${car.parking.id}`'><i
                                            ...
                                            <template v-if="car.parking.state == 'error'">
                                                <Tooltip content="Не оплачено" placement="top" :transfer="true">
                                                    <a v-if="$can('cars/parking')"
                                                       :data-remote='`/give/errorparkingaction/${car.parking.id}`'><i
                                                            class='fa fa-ban red-color'></i></a>
                                                    <i v-else class='fal fa-ban red-color'></i>
                                                </Tooltip>
                                            </template>

                                            <Tooltip content="Заврешение оплаты" placement="top" :transfer="true"
                                                     v-if="car.parking.state == 'need_stop'">
                                                <i class='fal fa-clock gray-color'></i>
                                            </Tooltip>

                                            <Tooltip content="В процессе оплаты" placement="top" :transfer="true"
                                        ...
                                        <div v-else>
                                            <Tooltip content="Моспаркинг" placement="top" :transfer="true">
                                                <a v-if="car.parkingType == 'allow_paid' && $can('cars/parking')"
                                                   :data-remote='`/give/gopay/${car.id}`'><i
                                                        class='fas fa-parking gray-light-color'></i></a>
                                            </Tooltip>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        ...
                                        <Tooltip :content="`ОСАГО до ${car.osago.date}`" placement="top" :transfer="true"
                                    ...
                                    <td>
                                        <Tooltip content="Нет ни одного штрафа" placement="top" :transfer="true"
                                                 v-if="car.penaltyTracking == 1">
                                            <i v-if="car.count_fines == 0"
                                               class="fad fa-traffic-light orange-color"></i>
                                        </Tooltip>
                                    ...
                                    <td align="center">
                                        ...
                                        <Tooltip content="Занято" placement="top" :transfer="true"
                                                 v-if="['1','7','8','9','16','18'].includes(car.status)">
                                            <div v-if="car.status == 1 && (($can('clients/info_client') && car.ses.isService == 0) || ($can('clients/info_service') && car.ses.isService == 1) || ($can('clients/info_owner') && car.ses.isService == 2))"
                                                 :data-orientation="car.ses.client.selfie_turn"
                                                 class="car_status car_status_big car_status_black"
                                                 :style="`background-image: url(/give/resizeimage?fn=${car.ses.client.selfie})`"></div>
                                        ...
                                        <Tooltip content="В резерве" placement="top" :transfer="true"
...
<!--                                    </td>-->
                                    ...
                                    <td>
                                        <Tooltip content="Информация" placement="top" :transfer="true"
                                    ...
                                    <td>
                                        <Tooltip content="История поездки" placement="top" :transfer="true"
                                    ...
                                    <td>
                                        <Tooltip content="Управление" placement="top" :transfer="true"
                                                 v-if="['1','12'].includes(car.status)">
                                            <Button class="ivu-btn-gray" shape="circle"
                                    ...
                                    <td>
                                        <Tooltip content="Трек поездки" placement="top" :transfer="true"
                                                 v-if="car.status == 1 && $can('trips/track')">
                                            <a target="_black" :href="`/devices/track/${car.ses.id}`"><i
                                                    class="fad fa-route gray-color"></i></a>
                                        </Tooltip>
                                    </td>

                                    <td>
                                        <Tooltip content="Скорость и обороты" placement="top" :transfer="true"
                                                 v-if="car.status == 1 && $can('trips/speed')">
                                            <a :data-remote="`/give/speeddrive/${car.ses.id}`"><i
                                                    class="fad fa-tachometer-alt gray-color"></i></a>
                                        </Tooltip>
                                    </td>
                                    ...
                                    <td>
                                        <template
                                            ...
                                            <a :href="`/devices/sos/${car.ses.client.id}`" type="button"
                                    ...
                                    <td>
                                        <Tooltip content="На карте" placement="top" :transfer="true"
                                                 v-if="type.sharingEnable == 1 && car.dLatitude != '0.00000000' && car.dLongitude != '0.00000000'">
<!--                                          :data-remote="`/give/infoclient/${car.ses.client.id}?mode=park&ses=${car.ses.id}`"-->
                                          <Button class="ivu-btn-gray" shape="circle" target="_blank" :to="`/map?device=${car.id}`">
                                    ...
                                    <td>
                                        <template v-if="['1','12'].includes(car.status) && car.ses.defect.show == 1">
                                            <Button shape="circle" :class="['ivu-btn-gray', car.ses.defect.class]"
                                    ...
                                    <td>
                                        <template
                                                v-if="$can('trips/compensation_create') && ['1','12'].includes(car.status)">

                                            <Tooltip content="Добавить компенсацию" placement="top" :transfer="true">
                                                <a data-target='#ModalDevice'
                                                   :data-remote='`/give/addcompensation/${car.ses.id}`'>
                                                    <Icon custom="far fa-plus orange-color"/>
                                                </a>
                                            </Tooltip>

                                            <Tooltip content="Компенсации" placement="top" :transfer="true">
                                                <a :data-remote='`/give/aboutcompensation/${car.ses.id}`'><span><b>{{ car.ses.compensation }}</b></span></a>
                                    ...
                                    <td>
                                        <template v-if="$can('deliveries/allow') && type.allowDelivery == 1">
                                            ...
                                            <Tooltip content="Доставка включена" placement="top" :transfer="true"
                                                     v-if="car.devAllowDelivery == 1">
                                                <Icon custom="fad fa-map-marked-alt orange-color"/>
                                            ...
                                            <Tooltip content="Доставка выключена" placement="top" :transfer="true" v-else>
                                                <Icon custom="fad fa-map gray-light-color"/>
                                    ...
                                    <td>
                                        <Tooltip content="Изменить радиус доставки" placement="top" :transfer="true"
                                                 v-if="$can('deliveries/allow_radius') && type.allowDelivery == 1 && car.devAllowDelivery == 1">
                                            <a class="changerangelink"
                                               :data-remote="`/give/changeradiusdelivery/${car.id}`">
                                    ...
                                    <td>
                                        <Tooltip content="Изменить время доставки" placement="top" :transfer="true"
                                    ...
                                    <td>
                                        ...
                                        <template v-if="$can('cars/available') && $can('cars/available_manager')">
                                            <Tooltip content="Доступ клиентам" placement="left" :transfer="true" v-if="car.status == 0">
                                        ...
                                        <template v-if="$can('trips/service') && ['1','12'].includes(car.status)">
                                            <Tooltip
                                                    ...
                                                    placement="left" :transfer="true" v-if="car.status == 0">
                                        ...
                                        <template
                                            ...
                                            <Tooltip content="Заблокировать" placement="top" :transfer="true" v-if="car.ses.stop == 0">
      ...
      <Modal v-model="test" title="Тачка" width="1000" :footer-hide="true">
        Тестируем модаль
      </Modal>
  ...
  import Mod from '@/components/Mod'
    ...
    export default {
        ...
        methods: {
            info_car(id) {
              this.test = true
              //   this.car_data.info.id = id
              //   this.car_data.info.show = true
            }
        },
