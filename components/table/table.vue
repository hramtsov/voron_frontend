<template>
   <div :class="wrapClasses" :style="styles" ref="tableWrap">
      <div :class="classes">
         ...
         <!-- <div
            v-for="column in columnRows[0]"
            :key="column.key"
            :name="`${column.key}_summary`"
            :test="`${column.key}_summary`"
         >{{ column }}</div>-->
...
<script>
// view-design/src/components/table/
import tableHead from "./table-head.vue";
import tableBody from "./table-body.vue";
import tableSummary from "./summary.vue";
import Spin from "view-design/src/components/spin/spin.vue"; // "../spin/spin.vue";
import {
   oneOf,
   getStyle,
   deepCopy,
   getScrollBarSize,
} from "view-design/src/utils/assist"; // "../../utils/assist";
import { on, off } from "view-design/src/utils/dom"; //  "../../utils/dom";

import Csv from "view-design/src/utils/csv"; //  '../../utils/csv';

import ExportCsv from "./export-csv";
import Locale from "view-design/src/mixins/locale"; //  "../../mixins/locale";
   ...
   methods: {
         ...
         this.data.forEach((item, index) => {
            ...
            if (
               ...
               selection = selection.concat(
                  this.getSelectionChildren(item, selection, selectionRowKeys)
               );
            }
         });
      ...
      filterData(data, column) {
         ...
         return data.filter((row) => {
            ...
            ///////////// МОЙ КОД
            if (column.filters_all) {
               let filters_value = column.filters_value
                  ? column.filters_value
                  : column.key;

               let filters_label = "";
               if (column.filters_label) {
                  let separator = column.filters_label_separator
                     ? column.filters_label_separator
                     : " ";
                  if (Array.isArray(column.filters_label)) {
                     for (var i = 0; i < column.filters_label.length; i++) {
                        if (
                           typeof row[column.filters_label[i]] !== "undefined"
                        ) {
                           if (filters_label.length) {
                              filters_label = filters_label + separator;
                           }
                           filters_label =
                              filters_label + row[column.filters_label[i]];
                        }
                     }
                  } else {
                     filters_label = row[column.filters_label];
                  }
               } else {
                  filters_label = row[filters_value];
               }

               let label =
                  row[filters_value] === null
                     ? column.filters_null
                        ? column.filters_null
                        : "null"
                     : filters_label;
               if (
                  !column.filters.filter(function (element) {
                     return element.value == row[filters_value];
                  }).length
               ) {
                  column.filters.push({
                     label: label,
                     value: row[filters_value],
                  });
               }
            }
            /////////////
            ...
            for (let i = 0; i < column._filterChecked.length; i++) {
               ...
               if (!column.filterMethod) {
                  ...
                  column.filterMethod = function (value, row) {
                     ...
                     return row[filters_value] == value;
                  };
               }
               ////////////
      ...
      makeData() {
         ...
         data.forEach((row, index) => {
            row._index = index;
            row._rowKey =
               typeof this.rowKey === "string" ? row[this.rowKey] : rowKey++;
