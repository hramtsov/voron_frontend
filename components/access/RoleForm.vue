<template>
    <Form :model="roleForm" ref="roleForm" :rules="roleRules" label-position="top">
        ...
        <Checkbox :indeterminate="indeterminate" :value="checkAll" @click.prevent.native="handleCheckAll">Выбрать все</Checkbox>
                            ...
                            <Col :xs="24" :md="12">
                                <Checkbox :label="prefix + `/` + key">
                                    <span class="label-checkbox">
                                        {{ permission.title }}
                                        <span v-if="permission.description" class="permission-description">{{ permission.description }}</span>
    ...
    export default {
        ...
        data() {
            return {
                ...
                roleRules: {
                    title: [
                        { required: true, message: 'Введите название роли', trigger: 'change' },
                    ],
                },
                ...
                this.$refs[name].validate(async (valid) => {
                    if (valid) {
                        ...
                        if(typeof this.$route.params.id === 'undefined'){
                            ...
                            try {
                                let c = await this.$axios.$post('/roles/edit/' + this.$route.params.id, this.roleForm)
                                this.$Notice.success({
